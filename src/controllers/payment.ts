...
export const createPaymentIntent = TryCatch(
  async (req, res, next) => {
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      amount: Number(amount) * 278,  //1 dollar = 278 Rs
      currency: "pkr",
    })
...
export const applyDiscount = TryCatch(
  async (req, res, next) => {
    ...
    return res.status(200).json({
      ...
      //sirf agar discount likhty tou saara data ye return krta discount object ka, isliye discount.amount likha hai takey sirf amount return krey
    })
  }
)
